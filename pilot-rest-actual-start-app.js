...
function handleFormChange(event) {
  ...
  if (
    ...
    (event.target === els.tocHours ||
      event.target === els.tocMinutes ||
      event.target === els.tocModifier)
  ) {
    syncActualStartToAdjustedToc();
  }
...
function syncActualStartToAdjustedToc() {
  const tocMinutes = getSelectedClockMinutes(els.tocHours, els.tocMinutes);
  const tocModifier = Number(els.tocModifier.value || 0);
  if (!Number.isFinite(tocMinutes) || !Number.isFinite(tocModifier)) {
    return;
  }
  setClockPicker(els.startHours, els.startMinutes, tocMinutes + tocModifier);
}

function readInputs() {
  return {
    flightDate: todayUtcIsoDate(),
    estimatedTocMinutes: getSelectedClockMinutes(els.tocHours, els.tocMinutes),
    estimatedTodMinutes: getSelectedClockMinutes(els.todHours, els.todMinutes),
    tocModifierMinutes: Number(els.tocModifier.value || 0),
    todModifierMinutes: Number(els.todModifier.value || 0),
    startMinutes: getSelectedClockMinutes(els.startHours, els.startMinutes),
...
function validateInputs(inputs) {
  ...
  if (!Number.isFinite(inputs.startMinutes)) {
    errors.push({ field: els.startHours, message: "Select a valid actual start time." });
  }
  if (
...
function getSelectedClockMinutes(hoursSelect, minutesSelect) {
  return Number(hoursSelect.value) * 60 + Number(minutesSelect.value);
}

function setClockPicker(hoursSelect, minutesSelect, totalMinutes) {
  const normalized = ((totalMinutes % MINUTES_PER_DAY) + MINUTES_PER_DAY) % MINUTES_PER_DAY;
  hoursSelect.value = String(Math.floor(normalized / 60));
  minutesSelect.value = String(normalized % 60);
}
